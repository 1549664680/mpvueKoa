...
module.exports = async (ctx) =>{
  ...
  for( let i = 0;i < categoryList.length; i++){
    ...
    //在商品中找到childCategoryIds里的7数据
    const categroyGoods = await mysql('nideshop_goods').column('id', 'name', 'list_pic_url', 'retail_price').whereIn('category_id',childCategoryIds).limit(7).select()
    newcategroyList.push({
      'id':item.id,
      'name':item.name,
      'goodsList':categroyGoods
    })
  }
  ctx.body ={
    'banner':banner,
    ...
    'hotGoods':hotGoods,
    'topicList':topicList,
    'newcategroyList':newcategroyList
  }
}
